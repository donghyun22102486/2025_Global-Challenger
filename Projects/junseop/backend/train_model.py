...
def train_and_save_model(data_path="../../datasets/Faults.NNA"):
    """
    ...
    try:
        ...
        print(
            f"오류: '{data_path}' 파일을 찾을 수 없습니다. UCI 웹사이트에서 다운로드하여 이 스크립트와 같은 폴더에 위치시켜 주세요."
        )
        return
    ...
    with open("../../models/fault_prediction_model.pkl", "wb") as f:
        pickle.dump(
            {"model": model, "label_encoder": le, "feature_names": feature_names}, f
        )
    print(
        "학습된 모델과 관련 정보가 'fault_prediction_model.pkl' 파일로 저장되었습니다."
    )
